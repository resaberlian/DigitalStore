...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_kamera', $this->input->post('kode_kamera')[$i])->get('kamera')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_kamera', $this->input->post('kode_kamera')[$i])->update('kamera', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_kamera'=>$this->input->post('kode_kamera')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('kamera', 'kamera.kode_kamera = detail_transaksi.kode_kamera')
